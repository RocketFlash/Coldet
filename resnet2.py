...
class RESNET:
    ...
    def __init__(self, output_directory, input_jacobians_shape, input_torques_shape, nb_classes, verbose=False):
        self.output_directory = output_directory
        self.model = self.build_model(input_jacobians_shape, nb_classes)
        if(verbose == True):
            self.model.summary()
        self.verbose = verbose
        self.model.save_weights(self.output_directory+'model_init.hdf5')

    def build_model(self, input_jacobians_shape, input_torques_shape, nb_classes):
        n_feature_maps = 64

        input_layer = keras.layers.Input(input_jacobians_shape)
